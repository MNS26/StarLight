...
void vulkan_framebuffer_create(
  ...
  // Take a copy of the renderpass, attachments and attachment count
  out_framebuffer->attachments = slallocate(sizeof(VkImageView), MEMORY_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context *context, vulkan_framebuffer *framebuffer) {
  ...
  if (framebuffer->attachments) {
    slfree(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMORY_TAG_RENDERER);
