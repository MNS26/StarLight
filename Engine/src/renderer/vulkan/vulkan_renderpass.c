...
void vulkan_renderpass_create(
  ...
  color_attachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // Dont expect any initial layout
  color_attachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // Transitioned to after render pass
  color_attachment.flags = 0;
  
  attachment_descriptions[0] = color_attachment;

  VkAttachmentReference color_attachment_reference;
  color_attachment_reference.attachment = 0; // Attachment to color index
  ...
  depth_attachment.format = context->device.debpth_format;
  ...
  // Attachments used for mulitsampling color attachments
...
void vulkan_renderpass_begin(vulkan_command_buffer *command_buffer, vulkan_renderpass *renderpass, VkFramebuffer framebuffer) {
  ...
  slzero_memory(clear_values, sizeof(VkClearValue)*2);
