...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
  ...
  for (u32 i = 0; i < context->device.swapchain_support.present_mode_count; ++i) {
    VkPresentModeKHR mode = context->device.swapchain_support.present_mode[i];
  ...
  // Clamp allowed value allowed by the GPU
  ...
  if (!swapchain->images)
    swapchain->images = (VkImage*)slallocate(sizeof(VkImage)*swapchain->image_count, MEMORY_TAG_RENDERER);

  VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handle, &swapchain->image_count, swapchain->images));

  // Views
  if (!swapchain->views)
    swapchain->views = (VkImageView*)slallocate(sizeof(VkImageView)*swapchain->image_count, MEMORY_TAG_RENDERER);
  ...
  if (!vulkan_device_detect_depth_format(&context->device)) {
    context->device.debpth_format = VK_FORMAT_UNDEFINED;
    SLFATAL("Failed to find supported depth format!");
  }
  ...
  SLINFO("Swapchain created sucessfully.");
}
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
  ...
  // Those are owned by the swapchain. he will get very angry if you yoink them.
  for (u32 i = 0; i < swapchain->image_count; ++i)
